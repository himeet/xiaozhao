...
##### 4.3.1 排他锁（X锁） Exclusive Locks（行锁）

- 排他锁又称写锁，对某一资源加排他锁，当前事务可以增删改查、其他事务无法进行任何操作
- 自动加锁：DML语句默认加排他锁（用于增删改表中数据，insert、update、delete）
- 手动加锁：SELECT * from user where id=1 for update;

##### 4.3.2 共享锁（S锁） Shared Locks（行锁）

- 共享锁又称读锁，可以同时加多个读锁，但是无法修改。需所有共享锁释放后才能修改。
- 加锁：select * from table lock in share mode;
- 解锁：commit, rollback

##### 4.3.3 意向锁 Intention Locks（表锁）

- **意向共享锁**（IS锁）Intention Shared Locks：事务准备给数据行加共享锁之前，要先获取该表的IS锁。
- **意向排他锁**（IX锁）Intention Exclusive Locks：事务准备给数据行加排他锁之前，要先获取该表的IX锁。
- **意向锁均为表锁，且无法手动创建**。其作用为：**统计是否有事务正在锁定该表的状态中，而不必遍历每一行。**

#### 4.4 锁的区间

![image-20210316201927887](/Users/gaolijing/Library/Application Support/typora-user-images/image-20210316201927887.png)

- Record：记录： 某条或某几条记录
- Gap：间隙：记录之间的间隙
- Next-Key：临键：一直锁到下一个键

#### 4.5 锁的算法

##### 4.5.1 Record Locks记录锁

只锁住一个个独立的元组。对于当前不存在的元组也可以锁！

##### 4.5.2 Gap Locks 间隙锁

- 只存在于Repeatable Read、Serializable隔离级别，只存在于MySQL
- Select * from user where id > 15 for update; 锁哪里？注意：间隙锁是根据已有元组划分区间的，所以锁的是(11, +无穷)

##### 4.5.3 Next-Key Locks 临键锁

- 如果锁的区间包含了某一个已有元组，就会触发临键锁
- 例如锁id > 5 and id < 10，包含元组9，实际锁的是(5, 11]

#### 4.6 其他锁算法

##### 4.6.1 插入意向锁 Insert Intension Locks

##### 4.6.2 自增锁 AUTO-INC Locks

- 默认模式1:两个事务，同时插入10条数据，编号可能是相互交叉的

##### 4.6.3 空间锁 Predicate Locks for Special Indexes
...
- InnoDB引擎在每个事务创建时，为它创建一个txID。每行数据有多个版本，在快照时，以最后一个已经commit提交的txID为标准
- 在开启事务后的第一次查询中，拍一张快照，之后再查询就使用**快照读**，所以可重复读，同时也貌似解决了“幻读”的问题
- 在快照之后，虽然查询使用“**快照读**”，但是更新时必须使用“**当前读**”，即先查询最新提交的事务的结果，再在这个结果上进行更新，所以这里并没有解决幻读的问题

### 6. 补充

#### 6.1 如果没有主键、也没建立唯一索引，会有什么问题？

- 对某个元组上锁的时候，导致整个表都会锁起来，而且还会遍历整个表，效率低下

- MySQL会自己加一个索引，占6字节

#### 6.2 行锁，锁什么？

锁的是唯一索引。如果A事务where条件没有唯一索引，那么整个表都会锁起来。以防B事务插入或删除一个符合where条件的元组，显得好像A事务没有上锁一样（其实A已经上锁了）。

#### 6.3 自增字段不是主键会怎么样？

如果autoinc的字段不是主键，则会在int资源耗尽时直接归零并正常insert。如果是主键或唯一索引会报错：主键冲突。
